package com.wxm.camerajob.data.define

/**
 * global define for app
 * Created by wxm on 2016/6/10.
 */
object GlobalDef {
    const val INTRET_CS_ACCEPT = 2010
    const val INTRET_CS_GIVEUP = 2011
    @Suppress("unused")
    const val INTRET_ACCEPT = 2020
    const val INTRET_GIVEUP = 2021
    const val INTRET_SURE = 2022

    const val INTRET_NOTCARE = 9000

    const val INT_INVALID_ID = -1
    const val INT_GLOBALJOB_PERIOD = 10000
    const val GLOBALJOB_CHECKPERIOD = 5

    const val INTRET_USR_LOGOUT = 2001

    const val MS_SECOND = 1000
    const val MS_MINUTE = 60000
    @Suppress("unused")
    const val MS_HOUR = 3600000

    const val STR_HELP_TYPE = "HELP_TYPE"
    const val STR_HELP_MAIN = "help_main"

    const val STR_CAMERA_DPI = "CAMERA_DPI"

    const val STR_CAMERAPROPERTIES_NAME = "camera_properties"
}


